...
async def set_system_prompt_api(request: Request):
    try:
        ...
        # Validate system_prompt - can be empty string
...
async def upload_documents_api(files: List[UploadFile] = File(...)):
    ...
    try:
        ...
        # Index the documents using a thread for synchronous indexing
...
async def toggle_rag_api(request: Request):
    try:
        ...
        if not isinstance(enable_rag, bool):
            raise HTTPException(status_code=400, detail="'enableRAG' must be a boolean value.")
        ...
        logger.error(f"Failed to toggle RAG: {e}")
        raise HTTPException(status_code=500, detail="Failed to toggle RAG.")
...
async def generate_response_api(request: PromptRequest):
    try:
        ...
        await services.set_generating_response(True)
        ...
        async def generate():
            try:
                ...
                await services.set_generating_response(False)
